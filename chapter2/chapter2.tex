...
\section{Results}
% RESULTS
% I. Identification and filtering of IDRs
%     A. From a set of over 8,500 alignments of single copy orthologs from the genomes of up to 33 species in the \textit{Drosophila} genus, we identified X regions with high levels of inferred disorder using the predictor AUCPreD. To highlight the unique features of IDR evolution in subsequent analyses, we also extracted a complementary set of Y regions with low levels of inferred disorder. Both sets were filtered on several criteria, including the lengths of their sequences and the phylogenetic diversity. In the subsequent discussion, we refer to these filtered sets as the ``disorder'' and ``order'' regions, respectively. The disorder
%     B. Some notes about the length distributions and other basic statistics
%
